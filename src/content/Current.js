const Current = {
  path: "/",
  content: {
    title: 'The Current Issue',
    photos: [
      { url: require("../images/sf/bart.jpg"),            aspect: "6-by-7" },
      { url: require("../images/sf/laundry.jpg"),         aspect: "3-by-2" },
      { url: require("../images/sf/sunset.jpg"),          aspect: "3-by-2" },
      { url: require("../images/sf/civic.jpg"),           aspect: "2-by-3" },
      { url: require("../images/sf/market.jpg"),          aspect: "6-by-6" },
      { url: require("../images/sf/mission.jpg"),         aspect: "3-by-2" },
      { url: require("../images/sf/muni.jpg"),            aspect: "2-by-3" },
      { url: require("../images/sf/car.jpg"),             aspect: "2-by-3" },
      { url: require("../images/la/echo.jpg"),            aspect: "3-by-2" },
      { url: require("../images/la/vinyl.jpg"),           aspect: "2-by-3" },
    ]
  }
}

export default Current;
